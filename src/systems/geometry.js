...
module.exports.System = registerSystem('geometry', {
  ...
  getOrCreateGeometry: function (data) {
    ...
    if (!data.useCache) { return createGeometry(data); }

    // Try to retrieve from cache first.
    hash = this.calculateHash(data);
    cachedGeometry = this.cache[hash];
    incrementCacheCount(hash);  // Whether get or create, we know to increase the count.
    ...
    this.cache[hash] = cachedGeometry;
  ...
  unuseGeometry: function (data) {
    ...
    var geometry;
    var hash = this.calculateHash(data);
